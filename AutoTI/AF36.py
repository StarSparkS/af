...
def main():
    ...
    # parser.add_argument('-3ml', action='store_true', help="Prints Custom Financial Report")

    #parser contract?
    args = parser.parse_args()
    logruns = open('RUN_LOG.csv', 'a')
    linr = str("Python 3.6   ") + str(datetime.now().strftime('%Y-%m-%d %H:%M:%S')) + "\t\t"
    ...
    #find start date
    start_Date = (list(map(min, list(zip(*(filter(lambda x: x[pd.runout].year > 1950, people))))))[pd.runout])+timedelta(days=1)
    #
    ...
    if(args.tf):
        ...
        fp = list(filter(lambda gto : gto[pd.contract] in contracts, people))
        people = fp
        linr = linr + "Contracts: \t" + args.tf + '\t\t'
    if(args.name):
        # print args.name.replace(".", " ")
        fpeople = list(filter(lambda gto: gto[pd.name] == str(args.name.replace(".", " ")), people))
        # print people[pd.name]
        people = fpeople
        linr = linr + "Person: \t" + args.name + '\t\t'
    if(args.comp):
        # print args.name.replace(".", " ")
        fpeople = list(filter(lambda gto: (gto[pd.company]).strip()== str(args.comp).strip(), people))
        # print people[pd.name]
        people = fpeople
    ...
    if(args.exl):
        # print args.name.replace(".", " ")
        fpeople =list(filter(lambda gto: gto[pd.company] != str(args.exl), people))
        # print people[pd.name]
        people = fpeople
        linr = linr + "Company Excluded: \t" + args.exl + '\t\t'
        # exit(1)
    team = list(filter(lambda x: len(x[pd.gna]) == 0, people))
    accountants = list(filter(lambda x: len(x[pd.gna]) >0, people))
        # exit(1)
    ...
    if (args.date): # Returns funding required to reach a certain date
        ...
        ngs = ["MIPR", "InHouse"]    # parse command line date/ create list of workdays between startdate and target date
        # note MIPR/INHOUSE not in GNA equation.
        try:
            SEDhrs = sum(list(zip(*(list(filter(lambda gto : gto[pd.center]=="SED" , people)))))[pd.hrs])
        except:
            SEDhrs = 0
        try:
            SED_G_mon = sum(list(zip(*((filter(lambda gto : gto[pd.center]=="SED" and gto[pd.contract] in ngs, people)))))[pd.val])
        except:
            SED_G_mon = 0 
        try:
            SED_ds_mon=  sum(list(zip(*(list(filter(lambda gto : gto[pd.center]=="SED" and gto[pd.contract] not in ngs, people)))))[pd.val])
        except:
            SED_ds_mon = 0
        try:
            SSDDhrs = sum(list(zip(*(list(filter(lambda gto : gto[pd.center]=="SSDD" , people)))))[pd.hrs])
        except:
            SSDDhrs = 0
        try:
            SSDD_G_mon = sum(list(zip(*((filter(lambda gto : gto[pd.center]=="SSDD" and gto[pd.contract] in ngs, people)))))[pd.val])
        except:
            SSDD_G_mon = 0
        try:
            SSDD_ds_mon=  sum(list(zip(*(list(filter(lambda gto : gto[pd.center]=="SSDD" and gto[pd.contract] not in ngs, people)))))[pd.val])
        except:
            SSDD_ds_mon = 0
        try:
            MDAhrs = sum(zip(*(list(filter(lambda gto : gto[pd.center]=="MDA", people))))[pd.hrs])
        except:
            MDAhrs = 0
        try:
            MDAmon=  sum(zip(*(list(filter(lambda gto : gto[pd.center]=="MDA", people))))[pd.val])
        except:
            MDAmon = 0
        print
        print (list(filter(lambda gto : gto[pd.center]=="SSDD" and gto[pd.contract] in ngs, people)))
        ...
        for n in clrs:
            print (n, "Team -", sum(list(zip(*(list(filter(lambda gto : gto[pd.color]== n, people)))))[pd.val]))
            nteam = list(filter(lambda gto : gto[pd.color] == n, people))
            for ac in cstrs:
                try:
                    accost = sum(list(zip(*(list(filter(lambda gto : gto[pd.customer]== ac, nteam)))))[pd.val])
                except:
                    accost = 0
                print  ("\t", ac, " - ", accost)
            if n == "Green":
                ...
                print (n, "Team -", sum(list(zip(*(list(filter(lambda gto : gto[pd.color]== n, people)))))[pd.val]) + (SED_ds_mon*Decimal(.03)).quantize(cent, rounding= ROUND_UP) +(SSDD_ds_mon*Decimal(.035)).quantize(cent, rounding= ROUND_UP))
                print ()
        print()
        ...
        #
        for to in tolist:
            tot = list(filter(lambda gto : gto[pd.contract]==to, people))
            hrs = sum(list(zip(*(tot)))[pd.hrs])
            # print (list(zip(*(tot)))[pd.val])
            mon=  sum(list(zip(*(tot)))[pd.val])
            ...
            for lis in alc:
                ...
                scc = list(filter( lambda x: all(f(x) for f in scf), tot ))
                print ("\t Category: ", lis)#, "\n")
                for row in scc:
                    print ("\t\t", row[pd.hrs], (5- len(str(row[pd.hrs])))*" ",  "\t", row[pd.val], (10- len(str(row[pd.val])))*" ", "."*6, row[0])
                print ("\t\t", "-"*16)
                print ("\t\t" ,sum(list(zip(*scc))[pd.hrs]), '\t', sum(list(zip(*scc))[pd.val]), "\n")
            print()
        print()
        ###########################  
    ...
    if (args.amt): # Returns runout date and hours based on given amount
        ...
        if(len(accountants)> 0):
            ...
            for pol in accountants:
                ...
                day = start_Date
                rate1 = list(filter( lambda x: all(f(x) for f in ratefilters), dateratelookup ))[0][rd.rate]
                pol[pd.tr] = rate1
            #
            ...
            if (day.weekday() < 5) and (day not in holidays): #if today is a working day ---> fund team
                ...
                #
                    ...
                    if(adf>0 and balance > 0): #This part is adjusting split 
                        spt = list(filter(lambda rt: (Decimal(rt[pd.loe]) >1.00 or Decimal(rt[pd.loe])<1.00) and (day>rt[pd.runout]), team))
                        for row in spt:
                        ...
                        if(balance <0):
                            ...
                            for row in rates:
                                if (row+balance >0):
                                    tu = list(filter(lambda a: a[pd.tr] == row, team))
                                    print (tu)
        ...
        rld = list(filter(lambda x : len(str(x))>0 , rld))
        # print rld
        ...
        ####
        adata = list(filter(lambda z:z[pd.runout] < day, team))
        maxwt = tb1
        maxwt = int(maxwt*100)
        mxh = 8
        if (args.rl):
        ...
        for row in aloc:
            ...
            v = sorted(list(filter(lambda x: all(f(x) for f in fr), team)), key = lambda x : x[pd.hrs])
            v[0][pd.hrs] += 1
...
def dates_to_funding(dateratelookup, start_Date, workdays, people ,team, accountants):
    ...
    for day in workdays:
        for pol in team:
            if day> pol[pd.runout] :
                ...
                lambda drl: drl[rd.dl]<=day, lambda drl: drl[rd.dg]>= day]
                try: 
                    rate = list(filter( lambda x: all(f(x) for f in ratefilters), dateratelookup ))[0][rd.rate]
                    pol[pd.tr] = rate
                except:
                    if(pol[pd.tr] == rr.rr):
                        rate = Decimal(rr.rr)
                        ...
                        lambda drl: drl[rd.lc] == pol[pd.lc],lambda drl: drl[rd.site] == pol[pd.site]]
                        dts = list(filter( lambda x: all(f(x) for f in rf2), dateratelookup))
    ...
    for to in tolist:
        hrs = sum(list(zip(*(list(filter(lambda gto : gto[pd.contract]==to, team)))))[pd.hrs])
        mon=  sum(list(zip(*(list(filter(lambda gto : gto[pd.contract]==to, team)))))[pd.val])
        ...
        tspend = 0
        toacts = list(filter(lambda acts: to in acts[pd.gna].split(" "), accountants)) #finds accountants related to contract
        while(gna2spend >0):
            if (tspend == gna2spend):
...
def dataMaker(people, spt):
    ...
    for i in lst.keys():
        ...
        for z in range (0, len(bnds)):
            ...
            check = list(filter( lambda x: all(f(x) for f in tocofilters), people ))
            #
            ...
            if(check):
                ...
                for raterow in raterows:
                    raterow = raterow.replace('\r', '').replace('\n', '').split("\t")
                    lc = raterow[0]
                    site = raterow[1]
                    jobfilters = [lambda a:a[pd.lc] == lc,  lambda b: b[pd.site]== site]
                    check2 = list(filter( lambda x: all(f(x) for f in jobfilters), check ))
                    #
