...
import requests
import argparse
import sys,os,csv,functools,operator,io,re
...
spt = r'C:\Users\PinnamaneniJa\Documents\budget\Rates'
holidays = []
...
### XXX: This code isn't good for anything besides USD at the moment.
### Maybe not even good for USD. Should really be using a the decimal
### module for this.
...
def parse_amount(amount_string):
    ...
    if amount_string == "-":
        return {}
    quantity = zero + Decimal(amount_string.translate(None,'$,'))*Decimal(100.0)

    # quantity = round(Decimal(amount_string.translate(None, "$,")) * Decimal(100.0))
...
def format_amount(amount):
    ...
    if units == 'USD':
        if quantity >= 0:
            return "${0:,.2f}".format(quantity/Decimal(100.0))
        else:
            return "-${0:,.2f}".format(-quantity/Decimal(100.0))
    #else:
...
def filter_by_date(transactions, first_date = None, last_date = None):
    if first_date:
        first_date = parsedate(first_date)
        transactions = [t for t in transactions if parsedate(t['date']) >= first_date]
    if last_date:
        last_date = parsedate(last_date)
        transactions = [t for t in transactions if parsedate(t['date']) <= last_date]
    return transactions
...
def reformat_date(date_string):
    ...
    return parsedate(date_string).isoformat()[:10]
...
def parse_first_line(line_number, line):
    ...
    # print "line: ", line, "\t", line_number
    difftrans.append(line_number)
    line = line.strip()
    split = line.split()
    date_string = split[0]
...
def parse_posting(line_number, line, adjust_sign):
    ...
    print amount_string
    print
    if("HR" in amount_string): #MagicRates
        amount_string = amount_string.strip("HR")
    print amount_string  
    if not is_valid_account_string(account_string):
        sys.stderr.write("Line %d: invalid account string: '%s'.\n" %
                         (line_number, account_string))
...
def workingdays(sdate,edd):
    ...
    return workdays
    # print workdays
def parse_postingv2(line_number, line, adjust_sign, currdate, currdesc):
    """parse string containing a posting.
    ...
    split = ns
    account_string = split[0]
    amount_string = split[1].strip(",").translate(None, "$")
    ...
    # print
    if("HR" in amount_string): #MagicRates
        print "\t _POST", account_string, amount_string, currdate
        amount_string = amount_string.strip("HR") # currently contains hours
        TO = account_string.split("TaskOrders")[1].split(":")[1] 
        CO = account_string.split("TaskOrders")[1].split(":")[2]
        ST = account_string.split("TaskOrders")[1].split(":")[3]
        LC = account_string.split("TaskOrders")[1].split(":")[4]
        sd = currdesc.split("[")[1].strip("]").split("-")[0]
        ed = currdesc.split("[")[1].strip("]").split("-")[1]
        sd = date(int(sd.split(":")[0]),int(sd.split(":")[1]),int(sd.split(":")[2]))
        ed = date(int(ed.split(":")[0]),int(ed.split(":")[1]),int(ed.split(":")[2]))
        workdays = workingdays(sd,ed)
        sh = Decimal(amount_string)/Decimal(len(workdays))
        print 
        print "\t\t", "TO:\t", TO
        print "\t\t", "CO:\t", CO
        print "\t\t", "ST:\t", ST
        ...
        print "\t\t", "SD:\t", sd
        print "\t\t", "ED:\t", ed
        print "\t\t", "WC:\t", len(workdays)
        print "\t\t", "HR:\t", amount_string
        print "\t\t", "SH:\t", sh
        # print currdesc
        # print
        fnm =  TO + ".txt"
                ...
                if(b==1):
                    ...
                    for c in tempdate:
                        if(len(c)>0):
                            w = c.split("-")
                            # print w
                            d1 = date(int(w[0].split("/")[2]),int(w[0].split("/")[0]),int(w[0].split("/")[1]))
                            d2 = date(int(w[1].split("/")[2]),int(w[1].split("/")[0]),int(w[1].split("/")[1]))
                            # print d1, d2
                            datestore.append((d1,d2))   
                    b = 0
                    # print datestore
                else:
                    arow= v.split("\t")
                    lcs[arow[0]] = {}
                    lcs[arow[0]][arow[1]] = arow[2:]
        ...
        # for day in workdays:
            ...
            # day = date, sh = hours on that day, to, co, st, lc
            # io.open(os.path.join(spt,TO))
...
def parse_transactions(lines, adjust_signs):
    ...
    print "Currently parsing from file"
    # print adjust_signs
    # print "currently parsing by line"
    transactions = []
    verify_balances = []
    transaction = {}
    postings = []
    ...
    currdesc = ""
    for line in lines:
        # print " looking at this line", line
        ...
        if len(line) == 0:
            ...
            if is_balance_verify_line(line):
                ...
                # print "Adjust Signs", adjust_signs
            elif not transaction:
                transaction = parse_first_line(line_count, line)
                currdate = transaction['date']
                currdesc = transaction['description']
                # print "parse_trans \t" ,line,"\n"
                print line,"\n"
            else:
                # print currdate
                postings += [parse_postingv2(line_count, line, adjust_signs, currdate,currdesc)]
    if transaction:
        transaction['postings'] = postings
        transactions += [transaction]
...
def print_register(transactions, account_string, include_related_postings, reverse_print_order, first_date, last_date):
    ...
    for line in data:
        print line
def thegoodprinter(desired, currs):
    spaces = ""
    xl = desired - currs
    for a in range(0,xl):
        spaces+=" "
    return spaces       
def thetoogoodprinter(desired, data):
    v = data.split(":")
    # print len(v), desired
    while (len(v)< desired):
...
def print_sheet(transactions, include_related_postings, reverse_print_order, first_date, last_date): 
    result = []
    levels = 0
    for regs in "ASSETS LIABILITIES REVENUE EXPENSES NET".split():
        # sdate,sdebit,scridit,saccount,sdesc = calculate_register(transactions, regs, include_related_postings, first_date, last_date)
    # C:\Users\PinnamaneniJa\Documents\ledger> C:\python27\python ledger_v1.py examples\test7.txt --print-balances --print-chart-of-accounts --print-register Assets --first-date 2017-09-01 --last-date 2018-08-01 --include-related-postings
        data = calculate_register(transactions, regs, include_related_postings, first_date, last_date)
        # data = rjust_column(data, 0)
        # data = rjust_column(data, 1)
        # data = rjust_column(data, 3)
        # data = ljust_column(data, 2)
        # print data.split('\t')
        # data = join_columns(data, '\t')
        ...
        if reverse_print_order:
            data.reverse()
        for d in data:
            if d[3].startswith('A') or d[3].startswith('E'):
                if d[2].startswith('-$'):
                    result.append(list([d[0], d[1],"", d[2],d[4]] + d[3].split(':')))
                else:
                    result.append(list([d[0], d[1],d[2],"",d[4]] + d[3].split(':')))
                ...
                if d[2].startswith('-$'):
                    ...
                    result.append(list([d[0], d[1],"",d[2],d[4]] + d[3].split(':')))
        if len(data):
            levels = max(levels,max(len(d[3].split(':')) for d in data))
    # levels = max(len(row) for row in result )
    # levels = 8
    # slis = [list(datetime.strptime(result[0][0], '%Y-%m-%d').date(), result[0][0])]
    thecsv = open(r'C:\Users\PinnamaneniJa\Documents\ledger\test7output',"w")
    cols = "Date, Assets, Expenses, Revenue, Liabilities, Amount, Debit, Credit, Description Hours\n"
    # thecsv.write(cols)
    ats2= liab2= rev2 =exp2 = Decimal(0.0)
    print 
    # print "Date\t\tAssets\t    Expenses\tRevenue\t    Liabilities\tAmount\t      Debit\t\t\t\t\t\t\t  Credit\t\t\t\t\t      Description"
    # print 225*"-"
    md = 0
    mc = 0
    for transaction in transactions:
        ...
        for posting in transaction['postings']:
            ...
            # print Decimal(posting['amount']['quantity']/100)
            amount = abs(Decimal(posting['amount']['quantity']/100))
            hold = Decimal(posting['amount']['quantity']/100)
            if("Assets" in posting['account']):
                ats2 +=hold
                if(hold > 0):
                    dc['debit'] = posting['account']
                    if md < len(posting['account'].split(':')):
                        md =len(posting['account'].split(':'))
                else:
                    dc['credit'] = posting['account']
                    if mc < len(posting['account'].split(':')):
                        mc =len(posting['account'].split(':'))
            if("Revenue" in posting['account']):
                rev2 += hold
                if(hold > 0):
                    dc['credit'] = posting['account']
                    if mc < len(posting['account'].split(':')):
            ...
            if("Liabilities" in posting['account']):
                liab2 +=hold
                if(hold > 0):
                    dc['credit'] = posting['account']
                    if mc < len(posting['account'].split(':')):
                    ...
                    if md < len(posting['account'].split(':')):
                        md =len(posting['account'].split(':'))
            # print posting, transaction['date']
        # print transaction['date']," |  ", ats2, thegoodprinter(6, len(str(ats2))), " + ", exp2, thegoodprinter(6, len(str(exp2))), " = " , rev2, thegoodprinter(6, len(str(rev2)))," + ", liab2, thegoodprinter( 8, len(str(liab2))), "  | ", amount, thegoodprinter(9, len(str(amount))), dc['debit'], thegoodprinter(50, len(dc['debit'])), dc['credit'], thegoodprinter(50, len(dc['credit'])),  transaction['description']
    # print md
    # print mc
    # print
    cols = 'Date\tAssets\tExpenses\tRevenue\tLiabilities\tAmount\tID\tDescription\t' + '\t'.join('Db%s' % i for i in range(md))+'\t'  +'\t'.join('Cr%s' % i for i in range(mc)) + "\n"
    print cols
    thecsv.write(cols)
    ats2= liab2= rev2 =exp2 = Decimal(0.0)
    print 
    print "Date\t\tAssets\t    Expenses\tRevenue\t    Liabilities\t    Amount\t\tID\t\tDebit\t\t\t\t\t\t\t  Credit\t\t\t\t\t      Description"
    print 225*"-"
        ...
        for posting in transaction['postings']:
            ...
            if("Liabilities" in posting['account']):
                ...
                if(hold > 0):
                    ...
                    dc['debit'] = posting['account']
            # print posting, transaction['date']
        ftran = transaction['description']
        ids = ""
        if ")" in transaction['description']:
            ids = transaction['description'].split(")")[0].strip("(")
            ftran =transaction['description'].split(")")[1]
        lwr = '\t'.join((transaction['date'],str(ats2), str(exp2), str(rev2), str(liab2), str(amount), ids,ftran )) + '\t' +thetoogoodprinter(md, dc['debit'] ) + '\t' +  thetoogoodprinter(mc, dc['credit'] ) + "\n"
        thecsv.write(lwr)
        print transaction['date']," |  ", ats2, thegoodprinter(6, len(str(ats2))), " + ", exp2, thegoodprinter(6, len(str(exp2))), " = " , rev2, thegoodprinter(6, len(str(rev2)))," + ", liab2, thegoodprinter( 8, len(str(liab2))), "  | ", amount,thegoodprinter(9, len(str(amount))), "  | ", ids,thegoodprinter(15, len(str(ids))) , dc['debit'], thegoodprinter(50, len(dc['debit'] )), dc['credit'], thegoodprinter(50, len( dc['credit'] )), transaction['description']
    # print md
    # print mc
    # ats= liab= rev =exp = Decimal(0.0)
    print
    print "ASSETS: ", ats2, " + EXPENSES: ", exp2, " = REVENUE: " , rev2, " + LIABILITIES", liab2
    # thecsv = open(r'C:\Users\PinnamaneniJa\Documents\ledgerout.csv',"w")
...
def main():
    ...
    parser.add_argument('--print-sheet',default=False,
                        action="store_true",
                        help='print the running balance for a specified account')
    ...
    print " IN main before parsing file"
    ...
    print
    print "#######################################"
    print "end of parsing and trans/verifications "
    print
    ...
    if (args.print_sheet):
        print "Command Line input successfully read"
        print_sheet(transactions, args.include_related_postings, args.reverse_print_order, args.first_date, args.last_date)
